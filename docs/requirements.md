...
| Performance targets | 50-100 users, <500ms search, 20-30 TCP clients, 10-20 chat users | Demo-supported / load evidence still limited | Basic indexes, offset/limit pagination, connection handling, targeted concurrency tests, and the API console are present; no external load-test artifact is committed. |
| Recovery use cases | DB cache/queue, TCP restart/queue, WebSocket client reconnect | Partial with demo paths | TCP queues up to 20 offline progress updates per user and replays them after auth reconnect; WebSocket keeps recent room history; HTTP health reports DB degradation; password recovery tokens are implemented for account recovery demos. |
| Bonus features | reviews, friends, stats, Redis cache, exports, streaming gRPC, delivery confirmation | Implemented where in current use-case scope | Advanced search parameters, reviews/ratings, friend requests/activity, personal stats, local catalog caching, and web demo pages are implemented. Redis remains represented by the in-process catalog cache for local course demo unless an external Redis service is added. |
